...
ggplot(multi_bismark_simp_stacked_sumr)+ geom_bar(aes(x = species, y = mean,fill = method,alpha = category, group = category),stat = "identity", position = position_dodge(), color = "black") + 
geom_errorbar(aes(x = species, ymin = mean-sd, ymax = mean+sd, group = category), width = 0.25,position=position_dodge(0.9), alpha = 1) + guides(color = guide_colorbar(order = 0), fill = guide_legend(order = 1)) + scale_alpha_manual(values=c(rev(seq(0.1,1, length.out = 6)))) + facet_wrap(~method, scale = "free")+theme_bw()+ theme(legend.position="top",legend.title = element_text(size = 10),
  legend.text = element_text(size = 8),axis.title.x=element_blank(),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,face = "italic"), strip.background =element_rect(fill="white")) + scale_fill_manual(values =c("#E6550D","#756BB1","#31A354")) + ylab("reads (average)")
dev.off()

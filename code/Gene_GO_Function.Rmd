...
Mcap.BPplot <- Mcap.GO.BP.long %>% filter(over_represented_pvalue<.01) %>%   ggplot(aes(x = name, y = reorder(GOterm_desc, ord))) +  
  ...
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
  strip.text.y.left = element_text(angle=0, size = 4, face = "bold"),
  strip.text.x = element_text(size = 12, face = "bold"),
  axis.title = element_blank(),
  axis.text.x = element_text(size = 24), 
  axis.text.y = element_text(size = 6), 
...
write.csv(Pact.GO.MF, "output/supplemental-material/ST8.csv")
write.csv(Pact.GO.BP, "output/supplemental-material/ST9.csv")
write.csv(Mcap.GO.MF, "output/supplemental-material/ST6.csv")
write.csv(Mcap.GO.BP, "output/supplemental-material/ST7.csv")

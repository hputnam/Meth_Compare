...
#add model column
aov_convr_lam_stats_t$model <- "conversion efficiency ~ library prep method"
```
...
#add model column
aov_convr_est_stats_t$model <- "conversion efficiency ~ library prep method"
```
...
```{r}
aov_convr_cmpr <- df_l_long %>% group_by(Species, Method) %>% do(aov_models = aov(perc_conv ~ convr_method, data =.))
...
#add group names back in
aov_convr_cmpr_stats <- cbind(aov_convr_cmpr[,1:2], aov_convr_cmpr_stats[,-1])

#transpose df
aov_convr_cmpr_stats_t <- setNames(data.frame(t(aov_convr_cmpr_stats[,-1])), aov_convr_cmpr_stats[,1:2])

#add model column
aov_convr_cmpr_stats_t$model <- "conversion efficiency ~ calculation method"
...
aov_convr_stats_all <- rbind(aov_convr_stats_t,aov_convr_cmpr_stats_t)

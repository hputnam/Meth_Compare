...
#create a df for plotting the num CpGs above the stacked bar plots
temp <- data.frame(methods=c("MBD", "RRBS", "WGBS", "MBD, RRBS","MBD, WGBS", "RRBS, WGBS", "MBD, RRBS, WGBS"), place=c(rep(1.02,7)), numObs=as.numeric(table(Pact_l_m_tag$group_simp)))
...
ggplot(Pact_l_m_tag,aes(x = perc_meth, fill = group_simp, color = "black")) + geom_histogram(binwidth = 10) + ylab("num CpGs") + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 60, hjust = 1), axis.title = element_text(size = 12, face = "bold")) + facet_wrap(~group_simp, scale = "free") + xlab("% methylation") + scale_color_manual(values = "black") + scale_fill_manual(values =c("red","red","red", "blue", "blue","blue", "black")) + theme(legend.position="none")
dev.off()
```
...
temp <- data.frame(methods=c("MBD", "RRBS", "WGBS", "MBD, RRBS","MBD, WGBS", "RRBS, WGBS", "MBD, RRBS, WGBS"), place=c(rep(1.02,7)), numObs=as.numeric(table(tail.Pact_l_m_tag$group_simp)))

ggplot(tail.Pact_l_m_tag,aes(x = group_simp, y = ..count.., fill = group_simp, alpha = tag )) + geom_bar(position = "fill", color = "black") + scale_y_continuous(labels=scales::percent) + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 60, hjust = 1), axis.title = element_text(size = 12, face = "bold")) + scale_fill_manual(values =c("red","red","red", "blue", "blue","blue", "black"))  + scale_alpha_manual(values=c(seq(0.1,1, length.out = 10)))+ labs(x = "method", y = "% CpGs",fill = "method", alpha = "% methylation")

#histograms
ggplot(tail.Pact_l_m_tag,aes(x = perc_meth, fill = group_simp, color = "black")) + geom_histogram(binwidth = 10) + ylab("num CpGs") + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 60, hjust = 1), axis.title = element_text(size = 12, face = "bold")) + facet_wrap(~group_simp, scale = "free") + xlab("% methylation") + scale_color_manual(values = "black") + scale_fill_manual(values =c("red","red","red", "blue", "blue","blue", "black")) + theme(legend.position="none")
```

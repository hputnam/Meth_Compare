...
```{r}
McapCpGLocationStatTest <- data.frame(t(McapFeatureOverlaps[,c(13, 9, 5)])) #Subset library preparation methods, transpose, and save as a dataframe
...
for(i in 1:ncol(McapCpGLocationStatTest)) { #For each genome feature
  Method1Feature <- McapCpGLocationStatTest[1,i] #Variable for # genome feature overlaps for method 1
  Method2Feature <- McapCpGLocationStatTest[2,i] #Variable for # genome feature overlaps for method 2
  Method1NotFeature <- sum(McapCpGLocationStatTest[1,-i]) #Variable for # other CpG types for method 1
  Method2NotFeature <- sum(McapCpGLocationStatTest[2,-i]) #Variable for # other CpG types for method 2
  ...
  rownames(ct) <- c(as.character(row.names(McapCpGLocationStatTest)[1:2])) #Assign row names: method 1, method 2
...
for(i in 1:ncol(McapCpGLocationStatTest)) { #For each genome feature
  Method1Feature <- McapCpGLocationStatTest[1,i] #Variable for # genome feature overlaps for method 1
  Method2Feature <- McapCpGLocationStatTest[3,i] #Variable for # genome feature overlaps for method 3
  Method1NotFeature <- sum(McapCpGLocationStatTest[1,-i]) #Variable for # other CpG types for method 1
  Method2NotFeature <- sum(McapCpGLocationStatTest[3,-i]) #Variable for # other CpG types for method 3
  ct <- matrix(c(Method1Feature, Method2Feature, Method1NotFeature, Method2NotFeature), ncol = 2) #Create contingency table
  colnames(ct) <- c(as.character(colnames(McapCpGLocationStatTest[i])), paste0("Not", colnames(McapCpGLocationStatTest[i]))) #Assign column names: type, not type
  rownames(ct) <- c(as.character(row.names(McapCpGLocationStatTest)[c(1,3)])) #Assign row names: method 1, method 3
  print(ct) #Confirm table is correct
  ctResults <- data.frame(broom::tidy(chisq.test(ct))) #Create dataframe storing chi-sq stats results. Use broom::tidy to extract results from test output
  ctResults$GenomeFeature <- as.character(colnames(McapCpGLocationStatTest)[i]) #Add CpG type to results
...
for(i in 1:ncol(McapCpGLocationStatTest)) { #For each genome feature
  Method1Feature <- McapCpGLocationStatTest[2,i] #Variable for # genome feature overlaps for method 2
  Method2Feature <- McapCpGLocationStatTest[3,i] #Variable for # genome feature overlaps for method 3
  Method1NotFeature <- sum(McapCpGLocationStatTest[2,-i]) #Variable for # other CpG types for method 2
  Method2NotFeature <- sum(McapCpGLocationStatTest[3,-i]) #Variable for # other CpG types for method 3
  ct <- matrix(c(Method1Feature, Method2Feature, Method1NotFeature, Method2NotFeature), ncol = 2) #Create contingency table
  colnames(ct) <- c(as.character(colnames(McapCpGLocationStatTest[i])), paste0("Not", colnames(McapCpGLocationStatTest[i]))) #Assign column names: type, not type
  rownames(ct) <- c(as.character(row.names(McapCpGLocationStatTest)[c(2,3)])) #Assign row names: method 2, method 3
  print(ct) #Confirm table is correct
  ctResults <- data.frame(broom::tidy(chisq.test(ct))) #Create dataframe storing chi-sq stats results. Use broom::tidy to extract results from test output
  ctResults$GenomeFeature <- as.character(colnames(McapCpGLocationStatTest)[i]) #Add CpG type to results
...
McapCpGLocationRRMB$comparison <- rep("WGBS vs. MBDBS", times = 5) #Add methods compared
...
```{r}
McapCpGLocationStatOutput <- cbind(McapCpGLocationWGRR,
                                   McapCpGLocationWGMB,
                                   McapCpGLocationRRMB) #Combine tables from each group of contingency tests
head(McapCpGLocationStatOutput)
```

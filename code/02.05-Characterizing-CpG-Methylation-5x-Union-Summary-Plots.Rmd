...
In this script, I'll create summary tables and figures to characterize CpG methylation in *M. capitata* and *P. acuta* 5x union bedgraphs for WGBS, RRBS, and MBDBS. I'll also use contingency tests to determine if there are any method-specific differences with CpG location in genomic features.
...
```{r}
McapAll <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-averages-counts.txt", header = FALSE, col.names = c("totalLines", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
McapAll <- McapAll[-c(4:5),] #Remove last rows (total lines for average information and for all files)
tail(McapAll) #Confirm import
```

```{r}
McapMeth <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-Meth-counts.txt", header = FALSE, col.names = c("Meth", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
McapMeth <- McapMeth[-c(4:5),] #Remove last row (total lines for all files)
tail(McapMeth) #Confirm import
```

```{r}
McapSparseMeth <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-sparseMeth-counts.txt", header = FALSE, col.names = c("sparseMeth", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
McapSparseMeth <- McapSparseMeth[-c(4:5),] #Remove last row (total lines for all files)
tail(McapSparseMeth) #Confirm import
```

```{r}
McapUnMeth <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-unMeth-counts.txt", header = FALSE, col.names = c("unMeth", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
McapUnMeth <- McapUnMeth[-c(4:5),] #Remove last row (total lines for all files)
...
```{r}
write.table(McapCpGType, "../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union-CpG-Type.txt", sep = "\t", quote = FALSE, row.names = TRUE) #Save summary table
```

### Create plot

```{r}
McapCpGTypePercents <- McapCpGType[,c(5,6,7)] #Keep only percent information
...
```{r}
#pdf("../output/supplemental-material/SF6A.pdf", height = 8.5, width = 11)
...
```{r}
PactAll <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Pact/Pact_union_5x-averages-counts.txt", header = FALSE, col.names = c("totalLines", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
PactAll <- PactAll[-c(4:5),] #Remove last row (total lines for all files)
tail(PactAll) #Confirm import
```

```{r}
PactMeth <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Pact/Pact_union_5x-Meth-counts.txt", header = FALSE, col.names = c("Meth", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
PactMeth <- PactMeth[-c(4:5),] #Remove last row (total lines for all files)
tail(PactMeth) #Confirm import
```

```{r}
PactSparseMeth <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Pact/Pact_union_5x-sparseMeth-counts.txt", header = FALSE, col.names = c("sparseMeth", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
PactSparseMeth <- PactSparseMeth[-c(4:5),] #Remove last row (total lines for all files)
tail(PactSparseMeth) #Confirm import
```

```{r}
PactUnMeth <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Pact/Pact_union_5x-unMeth-counts.txt", header = FALSE, col.names = c("unMeth", "filename")) #Import file using space as a delimiter. Columns are the number of lines and the filename
PactUnMeth <- PactUnMeth[-c(4:5),] #Remove last row (total lines for all files)
...
```{r}
write.table(PactCpGType, "../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Pact/Pact_union_5x-CpG-Type.txt", sep = "\t", quote = FALSE, row.names = TRUE) #Save summary table
```
...
```{r}
pdf("../output/supplemental-material/SF6B.pdf", height = 8.5, width = 11)
par(mar = c(2,5,0,1), oma = c(1, 1, 0, 12)) #Change figure boundaries
...
```{r}
pdf("../output/supplemental-material/SF6.pdf", height = 8.5, width = 11)
...
legend(x = 0.57, y = 0.90, 
       xpd = TRUE,
       legend = rev(c(expression("High (">="50%)"), "Moderate (10-50%)", expression("Low ("<="10%)"))),
       pch = 22, 
        col = "black", 
        pt.bg = rev(c("grey20", "grey50", "grey80")),
       bty = "n",
       cex = 1.5, 
       x.intersp = 0.7, xjust = 0) #Place a legend in the top right of the figure with no box
...
```{r}
McapGeneOverlaps <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-mcGenes-counts.txt", header = FALSE, col.names = c("counts", "filename")) #Import file with all file-gene overlaps
McapGeneOverlaps <- McapGeneOverlaps[-13,] #Remove final row
tail(McapGeneOverlaps) #Confirm import
```

```{r}
McapCDSOverlaps <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-mcCDS-counts.txt", header = FALSE, col.names = c("counts", "filename")) #Import file with all file-CDS overlaps
McapCDSOverlaps <- McapCDSOverlaps[-13,] #Remove final row
tail(McapCDSOverlaps) #Confirm import
```

```{r}
McapIntronsOverlaps <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-mcIntrons-counts.txt", header = FALSE, col.names = c("counts", "filename")) #Import file with all file-Introns overlaps
McapIntronsOverlaps <- McapIntronsOverlaps[-13,] #Remove final row
tail(McapIntronsOverlaps) #Confirm import
```

```{r}
McapFlanksOverlaps <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-mcFlanks-counts.txt", header = FALSE, col.names = c("counts", "filename")) #Import file with all file-Introns overlaps
McapFlanksOverlaps <- McapFlanksOverlaps[-13,] #Remove final row
tail(McapFlanksOverlaps) #Confirm import
```

```{r}
McapFlanksUpstreamOverlaps <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-mcFlanksUpstream-counts.txt", header = FALSE, col.names = c("counts", "filename")) #Import file with all file-Introns overlaps
McapFlanksUpstreamOverlaps <- McapFlanksUpstreamOverlaps[-13,] #Remove final row
tail(McapFlanksUpstreamOverlaps) #Confirm import
```

```{r}
McapFlanksDownstreamOverlaps <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-mcFlanksDownstream-counts.txt", header = FALSE, col.names = c("counts", "filename")) #Import file with all file-Introns overlaps
...
```{r}
McapIntergenicOverlaps <- read.table("../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union_5x-mcIntergenic-counts.txt", header = FALSE, col.names = c("counts", "filename")) #Import file with all file-Intergenic overlaps
...
```{r}
write.table(McapFeatureOverlaps, "../output/intermediate-files/02-Characterizing-CpG-Methylation-5x-Union/Mcap/Mcap_union-Genomic-Location-Counts.txt", sep = "\t", quote = FALSE, row.names = TRUE) #Save file
```
...
```{r}
McapCpGLocationStatTest <- data.frame(t(McapFeatureOverlaps[,c(1, 13, 9, 5)])) #Subset all CpG and library preparation methods, transpose, and save as a dataframe
McapCpGLocationStatTest <- McapCpGLocationStatTest[,-c(1, 4)] #Remove gene and flank information
head(McapCpGLocationStatTest) #Confirm formatting
```

#### All vs. WGBS

```{r}
McapCpGLocationAllWG <- data.frame() #Create empty dataframe to store chi-squared results
for(i in 1:ncol(McapCpGLocationStatTest)) { #For each genome feature
  Method1Feature <- McapCpGLocationStatTest[1,i] #Variable for # genome feature overlaps for genomic CpGs with data
  Method2Feature <- McapCpGLocationStatTest[2,i] #Variable for # genome feature overlaps for method 1
  Method1NotFeature <- sum(McapCpGLocationStatTest[1,-i]) #Variable for # other CpG types for genomic CpGs with data
  Method2NotFeature <- sum(McapCpGLocationStatTest[2,-i]) #Variable for # other CpG types for method 1
  ...
  rownames(ct) <- c(as.character(row.names(McapCpGLocationStatTest)[c(1,2)])) #Assign row names: genomic CpGs with data, method 1
  ...
  McapCpGLocationAllWG <- rbind(McapCpGLocationAllWG, ctResults) #Add test statistics to master table
}
```

```{r}
head(McapCpGLocationAllWG) #Look at statistical output
```

```{r}
McapCpGLocationAllWG$p.adj <- p.adjust(McapCpGLocationAllWG$p.value, method = "fdr") #Correct p-value using FDR
McapCpGLocationAllWG$comparison <- rep("all vs. WGBS", times = 5) #Add methods compared
head(McapCpGLocationAllWG) #Confirm changes
```

#### All vs. RRBS

```{r}
McapCpGLocationAllRR <- data.frame() #Create empty dataframe to store chi-squared results
for(i in 1:ncol(McapCpGLocationStatTest)) { #For each genome feature
  Method1Feature <- McapCpGLocationStatTest[1,i] #Variable for # genome feature overlaps for genomic CpGs with data
  ...
  ct <- matrix(c(Method1Feature, Method2Feature, Method1NotFeature, Method2NotFeature), ncol = 2) #Create contingency table
  colnames(ct) <- c(as.character(colnames(McapCpGLocationStatTest[i])), paste0("Not", colnames(McapCpGLocationStatTest[i]))) #Assign column names: type, not type
  rownames(ct) <- c(as.character(row.names(McapCpGLocationStatTest)[c(1,3)])) #Assign row names: genomic CpGs with data, method 2
  print(ct) #Confirm table is correct
  ctResults <- data.frame(broom::tidy(chisq.test(ct))) #Create dataframe storing chi-sq stats results. Use broom::tidy to extract results from test output
  ctResults$GenomeFeature <- as.character(colnames(McapCpGLocationStatTest)[i]) #Add CpG type to results
  McapCpGLocationAllRR <- rbind(McapCpGLocationAllRR, ctResults) #Add test statistics to master table
}
```

```{r}
head(McapCpGLocationAllRR) #Look at statistical output
```

```{r}
McapCpGLocationAllRR$p.adj <- p.adjust(McapCpGLocationAllRR$p.value, method = "fdr") #Correct p-value using FDR
McapCpGLocationAllRR$comparison <- rep("all vs. RRBS", times = 5) #Add methods compared
head(McapCpGLocationAllRR) #Confirm changes
```
...
```{r}
McapCpGLocationAllMB <- data.frame() #Create empty dataframe to store chi-squared results
for(i in 1:ncol(McapCpGLocationStatTest)) { #For each genome feature
  Method1Feature <- McapCpGLocationStatTest[1,i] #Variable for # genome feature overlaps for genomic CpGs with data
...
```{r}
pdf("../output/supplemental-material//Mcap_union-CpG-Features-MethStatus.pdf", height = 8.5, width = 11)
...
```{r}
#pdf("../output/supplemental-material/Pact_union-CpG-Features-MethStatus.pdf", height = 8.5, width = 11)
par(mar = c(1,5,0,1), oma = c(4, 1, 1, 11), mfcol = c(3,1)) #Change figure boundaries
...
```{r}
#pdf("../output/Fig_5.pdf", height = 8.5, width = 11)
par(mar = c(1,5,1,0), oma = c(2, 1, 0, 12), mfrow = c(2,1)) #Change figure boundaries
...
legend(x = 0.57, y = 0.91, 
       xpd = TRUE,
       legend = rev(c("CDS", "Introns", "Upstream Flank", "Downstream Flank", "Intergenic")),
       pch = 22, 
        col = "black", 
        pt.bg = rev(c("grey20", "grey40", "grey60", "grey80", "grey100")),
       bty = "n",
       cex = 1.5, 
       x.intersp = 0.7, xjust = 0) #Place a legend in the top right of the figure with no box

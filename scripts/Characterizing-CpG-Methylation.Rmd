...
```{r}
McapCpGMasterHigh <- (McapCpGMultiMaster[,-c(2:3)] / 100) #Remove moderate and low methylation columns. Divide by 100 to get dataframe with proportions, not percents
...
```{r}
McapCpGMasterModerate <- (McapCpGMultiMaster[,-c(1,3)] / 100) #Remove high and low methylation columns. Divide by 100 to get dataframe with proportions, not percents
...
```{r}
McapCpGMasterLow <- (McapCpGMultiMaster[,-c(1:2)] / 100) #Remove moderate and low methylation columns. Divide by 100 to get dataframe with proportions, not percents
...
Which genomic features affect methylation status?
...
summary(McapCpGHighModel) #Running the full model leads to a convergence issue. The model is likely overparametrized. I will run two separate models for each methylation status: one for genic regions (CDS and introns) and another for other regions (flanks and intergenic).
```
...
summary(McapCpGHighModel.Gene) #CDS and sequencing method significantly contribute to proportion of highly methylated CpGs. MBD-BS has higher prop highly methylated CpGs than RRBS and WGBS. Sig interaction between CDS and sequencing method for WGBS.
```
...
Disease.Treatment=glmmTMB(cbind(Total.Disease,Total.Coral-Total.Disease)~Treatment+(1|Site),family='binomial',data=SummedDiseaseData)
Disease.Null=glmmTMB(cbind(Total.Disease,Total.Coral-Total.Disease)~1+(1|Site),family='binomial',data=SummedDiseaseData)
anova(Disease.Treatment,Disease.Null,test='LRT')
summary(Disease.Treatment)

...
#McapCpGPercentsTrans <- data.trans((McapCpGMultiMaster / 100), method = "hellingers", plot = FALSE) #Hellinger (asymmetric) transformation
...
```{r}
nmds.scree(McapCpGPercentsTrans, distance = "euclidean", k = 10, autotransform = FALSE, trymax = 20) #Create a screeplot to compare the stress for solutions across different k values from 2 to 10. Use 20 different random start configurations. As the number of ordination axes increases, stress is minimized because the NMDS algorithm is trying to represent p dimensional data in k dimensions.
```

```{r}
McapCpGPercentsNMDS <- metaMDS(McapCpGPercentsTrans, distance = 'euclidean', k = 2, trymax = 10000, autotransform = FALSE) #Make MDS dissimilarity matrix on transformed data using euclidean distance.
McapCpGPercentsNMDS$stress #May have insufficient data since stress is close to 0
```

```{r}
nmds.monte(McapCpGPercentsTrans, distance = "euclidean", k = 2, autotransform = FALSE, trymax = 20) #Perform a randomization test to determine if the solution for k dimensions is significant. The observed stress value is less than the expected stress value.
stressplot(McapCpGPercentsNMDS) #Make Shepard plot to visualize the relationship between original dissimilarities (distance matrix) and distnaces in ordination space. The non-metric R-squared value is 1 (redundant with observed stress value and p-value from the randomization test)
```

```{r}
vec.McapCpGPercentsNMDS <- envfit(McapCpGPercentsNMDS$points, McapCpGPercentsTrans, perm = 1000) #Calculate loadings by correlating NMDS scores with original variables
vec.McapCpGPercentsNMDS #Look at loadings
```

```{r}
ordiplot(McapCpGPercentsNMDS, choices = c(1,2), type = "text", display = "sites", xlab = "Axis 1", ylab = "Axis 2") #Plot basic NMDS
plot(vec.McapCpGPercentsNMDS, p.max = 0.05, col = 'blue') #Plot loadings that are significant at the 0.05 level
```

```{r}
McapCpGPercentsNMDS$points #Samples from each method overlap with eachother
```
...
```{r}
McapCpGPercentsANOSIM <- anosim(dissimMcapCpGPercentsTrans, grouping = ANOSIMReplicates) #Conduct ANOSIM by method
McapCpGPercentsANOSIM$statistic #Within group and between group similarities are different for each sequencing method
McapCpGPercentsANOSIM$signif #Differences are significant
plot.anosim(McapCpGPercentsANOSIM) #Obtain boxplot of between and within group similarities
```
...
McapCpGPercentsWGRRANOSIM$signif #Differences are not significant
plot.anosim(McapCpGPercentsWGRRANOSIM)
```
...
```{r}
McapCpGPercentsWGMBANOSIM <- anosim(McapCpGPercentsWGMB, grouping = ANOSIMReplicates[c(1:3, 7:9)]) #Conduct pairwise ANOSIM for WGBS and MBD-BS data. Only use sequencing method metadata for these samples
McapCpGPercentsWGMBANOSIM$statistic
McapCpGPercentsWGMBANOSIM$signif #Differences are not significant
plot.anosim(McapCpGPercentsWGMBANOSIM)
```
...
```{r}
McapCpGPercentsRRMBANOSIM <- anosim(McapCpGPercentsRRMB, grouping = ANOSIMReplicates[c(4:9)]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
McapCpGPercentsRRMBANOSIM$statistic
McapCpGPercentsRRMBANOSIM$signif #Differences are not significant
plot.anosim(McapCpGPercentsRRMBANOSIM)
```
...
#PactCpGPercentsTrans <- data.trans((PactCpGMultiMaster / 100), method = "hellingers", plot = FALSE) #Hellinger (asymmetric) transformation
...
```{r}
nmds.scree(PactCpGPercentsTrans, distance = "euclidean", k = 10, autotransform = FALSE, trymax = 20) #Create a screeplot to compare the stress for solutions across different k values from 2 to 10. Use 20 different random start configurations. As the number of ordination axes increases, stress is minimized because the NMDS algorithm is trying to represent p dimensional data in k dimensions.
```

```{r}
PactCpGPercentsNMDS <- metaMDS(PactCpGPercentsTrans, distance = 'euclidean', k = 2, trymax = 10000, autotransform = FALSE) #Make MDS dissimilarity matrix on transformed data using euclidean distance.
PactCpGPercentsNMDS$stress #May have insufficient data since stress is close to 0
```

```{r}
nmds.monte(PactCpGPercentsTrans, distance = "euclidean", k = 2, autotransform = FALSE, trymax = 20) #Perform a randomization test to determine if the solution for k dimensions is significant. The observed stress value is less than the expected stress value.
stressplot(PactCpGPercentsNMDS) #Make Shepard plot to visualize the relationship between original dissimilarities (distance matrix) and distnaces in ordination space. The non-metric R-squared value is 1 (redundant with observed stress value and p-value from the randomization test)
```

```{r}
vec.PactCpGPercentsNMDS <- envfit(PactCpGPercentsNMDS$points, PactCpGPercentsTrans, perm = 1000) #Calculate loadings by correlating NMDS scores with original variables
vec.PactCpGPercentsNMDS #Look at loadings
```

```{r}
ordiplot(PactCpGPercentsNMDS, choices = c(1,2), type = "text", display = "sites", xlab = "Axis 1", ylab = "Axis 2") #Plot basic NMDS
plot(vec.PactCpGPercentsNMDS, p.max = 0.05, col = 'blue') #Plot loadings that are significant at the 0.05 level
```

```{r}
PactCpGPercentsNMDS$points #Samples from WGBS and RRBS overlap with eachother. Meth8 may be an MBD-BS outlier
```
...
```{r}
PactCpGPercentsANOSIM <- anosim(dissimPactCpGPercentsTrans, grouping = ANOSIMReplicates) #Conduct ANOSIM by method
PactCpGPercentsANOSIM$statistic #Within group and between group similarities are different for each sequencing method
PactCpGPercentsANOSIM$signif #Differences are significant
plot.anosim(PactCpGPercentsANOSIM) #Obtain boxplot of between and within group similarities
```
...
```{r}
PactCpGPercentsWGRRANOSIM <- anosim(PactCpGPercentsWGRR, grouping = ANOSIMReplicates[1:6]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
PactCpGPercentsWGRRANOSIM$statistic
PactCpGPercentsWGRRANOSIM$signif #Differences are not significant
plot.anosim(PactCpGPercentsWGRRANOSIM)
```
...
```{r}
PactCpGPercentsWGMBANOSIM <- anosim(PactCpGPercentsWGMB, grouping = ANOSIMReplicates[c(1:3, 7:9)]) #Conduct pairwise ANOSIM for WGBS and MBD-BS data. Only use sequencing method metadata for these samples
PactCpGPercentsWGMBANOSIM$statistic
PactCpGPercentsWGMBANOSIM$signif #Differences are not significant
plot.anosim(PactCpGPercentsWGMBANOSIM)
```
...
```{r}
PactCpGPercentsRRMBANOSIM <- anosim(PactCpGPercentsRRMB, grouping = ANOSIMReplicates[c(4:9)]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
PactCpGPercentsRRMBANOSIM$statistic
PactCpGPercentsRRMBANOSIM$signif #Differences are not significant
plot.anosim(PactCpGPercentsRRMBANOSIM)
```
...
```{r}
McapFeatureOverlapsPercentsANOSIM <- anosim(dissimMcapFeatureOverlapsPercentsTrans, grouping = ANOSIMReplicates) #Conduct ANOSIM by method
...
```{r}
McapFeatureOverlapsPercentsWGRRANOSIM <- anosim(McapFeatureOverlapsPercentsWGRR, grouping = ANOSIMReplicates[1:6]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
...
```{r}
McapFeatureOverlapsPercentsWGMBANOSIM <- anosim(McapFeatureOverlapsPercentsWGMB, grouping = ANOSIMReplicates[c(1:3, 7:9)]) #Conduct pairwise ANOSIM for WGBS and MBD-BS data. Only use sequencing method metadata for these samples
...
```{r}
McapFeatureOverlapsPercentsRRMBANOSIM <- anosim(McapFeatureOverlapsPercentsRRMB, grouping = ANOSIMReplicates[c(4:9)]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
...
```{r}
PactFeatureOverlapsPercentsANOSIM <- anosim(dissimPactFeatureOverlapsPercentsTrans, grouping = ANOSIMReplicates) #Conduct ANOSIM by method
...
```{r}
PactFeatureOverlapsPercentsWGRRANOSIM <- anosim(PactFeatureOverlapsPercentsWGRR, grouping = ANOSIMReplicates[1:6]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
...
```{r}
PactFeatureOverlapsPercentsWGMBANOSIM <- anosim(PactFeatureOverlapsPercentsWGMB, grouping = ANOSIMReplicates[c(1:3, 7:9)]) #Conduct pairwise ANOSIM for WGBS and MBD-BS data. Only use sequencing method metadata for these samples
...
```{r}
PactFeatureOverlapsPercentsRRMBANOSIM <- anosim(PactFeatureOverlapsPercentsRRMB, grouping = ANOSIMReplicates[c(4:9)]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples

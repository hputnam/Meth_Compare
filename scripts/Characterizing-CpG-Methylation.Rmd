...
```{r}
nmds.monte(McapCpGMultiTrans, distance = "euclidean", k = 2, autotransform = FALSE, trymax = 20) #Perform a randomization test to determine if the solution for k dimensions is significant. The observed stress value, 0.0000703346, is less than the expected stress value. P-value = 0.0198019802
...
```{r}
McapMasterNMDS$points #Methods appear to overlap with eachother
```
...
```{r}
methodANOSIM <- anosim(dissimMcapCpGMultiTrans, grouping = ANOSIMReplicates) #Conduct ANOSIM by method
methodANOSIM$statistic #Within group and between group similarities are different for each sequencing method
methodANOSIM$signif #Differences are significant
plot.anosim(methodANOSIM) #Obtain boxplot of between and within group similarities
```
...
```{r}
dataWGRR <- vegdist(McapCpGMultiTrans[c(1:3, 4:6),], "euclidean") #Subset WGBS and RRBS data and calculate dissimilarity matrix
```

```{r}
WGRRANOSIM <- anosim(dataWGRR, grouping = ANOSIMReplicates[1:6]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
WGRRANOSIM$statistic
WGRRANOSIM$signif #Differences are not significant
plot.anosim(WGRRANOSIM)
```

##### WGBS vs. MBD-BS

```{r}
dataWGMB <- vegdist(McapCpGMultiTrans[c(1:3, 7:9),], "euclidean") #Subset WGBS and MBD-BS data and calculate dissimilarity matrix
```

```{r}
WGMBANOSIM <- anosim(dataWGMB, grouping = ANOSIMReplicates[c(1:3, 7:9)]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
WGMBANOSIM$statistic
WGMBANOSIM$signif #Differences are not significant
plot.anosim(WGMBANOSIM)
```

##### RRBS vs. MBD-BS

```{r}
dataRRMB <- vegdist(McapCpGMultiTrans[c(4:9),], "euclidean") #Subset RRBS and MBD-BS data and calculate dissimilarity matrix
```

```{r}
RRMBANOSIM <- anosim(dataRRMB, grouping = ANOSIMReplicates[c(4:9)]) #Conduct pairwise ANOSIM for WGBS and RRBS data. Only use sequencing method metadata for these samples
RRMBANOSIM$statistic
RRMBANOSIM$signif #Differences are not significant
plot.anosim(RRMBANOSIM)
```

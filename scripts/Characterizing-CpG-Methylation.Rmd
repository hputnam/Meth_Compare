...
```{r}
barplot(t(McapCpGTypeTotalPercents), col='white', axes=F, axisnames=F, yaxp=c(0,1,2), las=1)
for (i in 1:ncol(t(McapCpGTypeTotalPercents))) {
    xx = t(McapCpGTypeTotalPercents)
    xx[,-i] <- NA
    colnames(xx)[-i] <- NA
    barplot(xx,col=dichromat(c(plotColors[i], plotColors[2*i], plotColors[i*3])), add=T, axes=F) 
}
```

...
```{r}
McapFeatureOverlaps <- data.frame("allCpGs" = rep(0, times = 6),
                                  "MBDBSMeth" = rep(0, times = 6),
                                  "MBDBSsparseMeth" = rep(0, times = 6),
                                  "MBDBSunMeth" = rep(0, times = 6),
                                  "MBDBS" = rep(0, times = 6),
                                  "RRBSMeth" = rep(0, times = 6),
                                  "RRBSsparseMeth" = rep(0, times = 6),
                                  "RRBSunMeth" = rep(0, times = 6),
                                  "RRBS" = rep(0, times = 6),
                                  "WGBSMeth" = rep(0, times = 6),
                                  "WGBSsparseMeth" = rep(0, times = 6),
                                  "WGBSunMeth" = rep(0, times = 6),                                  
                                  "WGBS" = rep(0, times = 6)) #Create blank dataframe with information for various CpG categories and methylation status. Match columns to the order of columns in the overlap count files
row.names(McapFeatureOverlaps) <- c("Genes", "CDS", "Introns", "Flanks", "Intergenic", "totalLines") #Assign row names
...
McapFeatureOverlaps$allCpGs <- c(McapGenomeFeatures$counts[3],
                                 ...
                                 0) #Assign information for CG motif overlaps with genome features. Use 0 for totalLines
...
for (i in 1:length(McapFeatureOverlaps)) {
  McapFeatureOverlaps[6,i] <- sum(McapFeatureOverlaps[,i])
} #Add the counts in each column and save in a new row
head(McapFeatureOverlaps) #Check addition
```
...
for (i in 1:length(McapFeatureOverlaps)) {
  McapFeatureOverlapsPercents[i,] <- (McapFeatureOverlaps[i,] / McapFeatureOverlaps[6,]) * 100
} #Divide every entry by totalLines and multiply by 100 to get percentages
McapFeatureOverlapsPercents <- McapFeatureOverlapsPercents[1:5,] #Remove totalLines row
...
```{r}
McapCpGTypeWGRR <- chisq.test(data.frame(t(McapCpGTypePercents[c(1, 2),]))) #WGBS vs. RRBS
McapCpGTypeWGRR #The distribution of CpGs is not sginificantly different between WGBS and RRBS
```

```{r}
McapCpGTypeWGMB <- chisq.test(data.frame(t(McapCpGTypePercents[c(1, 3),]))) #WGBS vs. MBD-BSSeq
McapCpGTypeWGMB #The distribution of CpGs is sginificantly different between WGBS and MBD-BSSeq
```

```{r}
McapCpGTypeRRMB <- chisq.test(data.frame(t(McapCpGTypePercents[c(2, 3),]))) #RRBS vs. MBD-BSSeq
...
```{r}
PactCpGTypeWGRR <- chisq.test(data.frame(t(PactCpGTypePercents[c(1, 2),]))) #WGBS vs. RRBS
PactCpGTypeWGRR #The distribution of CpGs is not sginificantly different between WGBS and RRBS
```

```{r}
PactCpGTypeWGMB <- chisq.test(data.frame(t(PactCpGTypePercents[c(1, 3),]))) #WGBS vs. MBD-BSSeq
PactCpGTypeWGMB #The distribution of CpGs is sginificantly different between WGBS and MBD-BSSeq
```

```{r}
PactCpGTypeRRMB <- chisq.test(data.frame(t(PactCpGTypePercents[c(2, 3),]))) #RRBS vs. MBD-BSSeq
...
```{r}
WGBSCpGType <- chisq.test(cbind(t(McapCpGTypePercents)[,1], t(PactCpGTypePercents)[,1]))
WGBSCpGType #The distribution of CpGs for WGBS is not significantly different between species
```

```{r}
RRBSCpGType <- chisq.test(cbind(t(McapCpGTypePercents)[,2], t(PactCpGTypePercents)[,2]))
RRBSCpGType #The distribution of CpGs for RRBS is significantly different between species
```

```{r}
MBDCpGType <- chisq.test(cbind(t(McapCpGTypePercents)[,3], t(PactCpGTypePercents)[,3]))

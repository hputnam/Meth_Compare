...
xxx <-Mcap.genes[duplicated(Mcap.genes$gene),]
...
#NEED TO GENERATE HISTOGRAM AND RELATIVE FREQUENCY
par(mfrow=c(1,3))
histogram(Mcap.WGBS.RelFreq$plusone, type = "percent",  #Specify the variable and type of histogram
          main = "Mcap expression WGBS",  #Create a title for the chart
          xlab = "Counts (log10(TPM+1))",  #Label the x-axis. Include units!
          ylab = "Relative Frequency",
          ylim = c(0,6),
          breaks=seq(from=0,to=6,by=0.1),
          col = "green")

histogram(Mcap.RRBS.RelFreq$plusone, type = "percent",  #Specify the variable and type of histogram
          main = "Mcap expression RRBS",  #Create a title for the chart
          xlab = "Counts (log10(TPM+1))",  #Label the x-axis. Include units!
          ylab = "Relative Frequency",
          ylim = c(0,6),
          breaks=seq(from=0,to=6,by=0.1),
...
histogram(Mcap.MBDBS.RelFreq$plusone, type = "percent",  #Specify the variable and type of histogram
          main = "Mcap expression MBDBD",  #Create a title for the chart
          xlab = "Counts (log10(TPM+1))",  #Label the x-axis. Include units!
          ylab = "Relative Frequency",
          ylim = c(0,6),
          breaks=seq(from=0,to=6,by=0.1),
...
Pact_presentexpression_by_method <- ggplot(Pact.CpG.counts.present , aes(plusone, color = method)) +
  ...
  ggtitle("C) Expression of Present Genes") +
...
Pact_absentexpression_by_method <- ggplot(Pact.CpG.counts.absent , aes(plusone, color = method)) +
  coord_trans(x="log") +
  geom_density(fill = NA)+
  ggtitle("D) Expression of Absent Genes") +
  xlab("Log of Mean Counts +1") + #label x axis
  theme(axis.text.x = element_text(angle = 90, size=10)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))

...
Goal is to identify and compare presence and absence number of CpG and the expression levels of the genes captured by each method
...
ggplot(Ortho.data.counts, aes(ID, method, fill=prop, width = 1)) + 
  geom_tile() +
...
ggplot(Pact.Ortho.data.counts, aes(ID, method, fill=prop, width = 1)) + 
  geom_tile() +
  scale_fill_gradient2(low="cyan", mid="magenta", high="yellow", na.value = "black", midpoint=0.5)+
  theme(axis.text.x = element_text(angle = 90, size=1)) +
  ggtitle("B P. acuta")
...
pdf("Output/Pact_PresAbs_genes.pdf")
...
plot_grid(Mcap.PresAbs, Pact.PresAbs,  ncol=1, nrow=2, align="v", labels = c('A) M. Capitata', 'B) P. acuta', '', ''), label_size = 12)
ggsave("Output/PresAbs_heatmaps.pdf", width=12, height=6)
